...
mdview —— 用 Tauri 2 打造的 Markdown 预览/编辑桌面小工具。
...
- **开发**：`npm run tauri dev`；**打包**：`npm run tauri build`（产物在 `src-tauri/target/release/`，含 `mdview.exe` 与 `bundle/` 下的 msi/nsis）。
...
- **发布**：源码推 GitHub（`caoqianming/mdview`，HTTPS），二进制作为 **Release 资产**上传（不进 git 历史）。
