...
### 4.19 外部改动检测（窗口聚焦时重载，v0.3.1）

运行期间别的编辑器改了已打开的文件，Dotdown 不会主动收到通知。方案：监听窗口
`onFocusChanged`，**窗口重新聚焦时**调 `checkExternalChanges()` 逐个比对已打开文件——
读磁盘内容（统一 EOL 后）与 `tab.lastSaved` 比较，相同则跳过；不同则：

- **干净标签**（无未保存修改）：`reloadTabFromDisk` 静默重载——换入磁盘内容的新 state，
  `lastSaved` 对齐磁盘。激活标签同时刷新预览与字数。
- **冲突标签**（有未保存修改且磁盘也变了）：弹 `ask` 让用户选「重载（放弃本地）」或
  「保留本地修改」。保留时把 `lastSaved` 记为当前磁盘内容，避免每次聚焦重复弹窗。

`checkingExternal` 互斥防重入；遍历前 `syncActive()` 把激活标签写回、用标签数组快照
