...
- 批量关闭逐个复用 `closeTab`（未保存的逐个弹确认，全关后自动留一个空白标签）。
- 菜单贴鼠标定位并防出界，点击外部 / Esc / 窗口失焦即关闭；样式沿用下拉菜单变量，
  深浅主题通用。

## 2026-07-10

### 新功能：右键「新建」子菜单出现 Markdown 项
- `installer-hooks.nsh` 注册 `.md\ShellNew`（NullFile）：资源管理器右键 → 新建 →
  Markdown，建空文件原地改名，双击按默认程序打开。卸载钩子同步清理。
- 首版还做了目录右键「新建 Markdown 并打开」（`Directory\shell` + 后端 `--new <目录>`
  参数一步建档并启动），经讨论**砍掉**：Win11 精简菜单埋得深，且与「新建→双击」的
  系统习惯重复；Rust 侧相应逻辑一并回退，零残留。
- 注：ShellNew 随安装包写注册表，需**安装新包后生效**；未出现可重启资源管理器。
...
- **滚动同步重写**：原纯比例映射在 KaTeX/Mermaid 引入后严重失真（5 行源码渲染出几百像素图表）。
  改为相邻锚点间线性插值，并补上**预览 → 编辑器反向同步**；顶/底强制对齐、无锚点回退比例、
  仅分栏模式启用。`claimScroll` owner 锁防两侧回环互拽。
- **大纲点击**：除滚动预览外，同时把编辑器光标定位到对应源码行（`suspendScrollSync` 防互拽）。
- `rawImagePaths` 更名 `exportRender`：导出渲染统一跳过图片重写与 `data-line` 注入。
- 详见 `design.md §4.7`。`tsc` ✅、`vite build` ✅。版本 → v0.3.4。

## 2026-06-29（续）

### 已发布：v0.3.3 双平台
- 源码 + tag `v0.3.3` 推 GitHub 与 Gitee；`tauri build` 出 `Dotdown_0.3.3_x64-setup.exe`（NSIS 4.53MB）
  与免安装 `dotdown.exe`（11.56MB）——两者较前明显增大，因引入 Mermaid/KaTeX。
- 两端 Release 均经 REST API 创建并上传两资产；本次用 **PowerShell `Invoke-RestMethod`**
  （GitHub token 取自 git credential、Gitee 读 `GITEE_TOKEN`），JSON body 直接成功、中文不乱码，
  比以往 curl/Python 绕法干净。
...
- 干净标签静默重载（换入磁盘内容、`lastSaved` 对齐）；有未保存修改的冲突标签弹窗让用户选
  「重载（放弃本地）」或「保留本地修改」（保留时记下磁盘状态避免重复弹窗）。
- `checkingExternal` 防重入、标签数组快照防 await 期间变动、文件读不到则忽略。详见 `design.md §4.19`。
- `tsc` ✅。版本 → v0.3.1。
